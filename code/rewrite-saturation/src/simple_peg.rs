...
impl fmt::Display for Statement {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Statement::Seq(ref statements) => if statements.is_empty() {
                write!(formatter, "{}", "{}")
            ...
            Statement::Assign(ref name, ref expr) => write!(formatter, "{} := {}", name, expr),
            Statement::If(ref cond, ref then_statement, ref else_statement) => write!(
                formatter,
                "if({}) {{ {} }} else {{ {} }}",
                cond,
                *then_statement,
            ...
            Statement::While(ref cond, ref body_statement) => {
                write!(formatter, "while({}) {{ {} }}", cond, *body_statement)
            }
        }
    }
}
...
impl fmt::Display for Expression {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Expression::Ref(ref name) => write!(formatter, "{}", name),
            Expression::Apply(ref func, ref args) => if args.is_empty() {
...
mod tests {
    use super::*;
    use super::Statement::{Assign, If, Seq, While};
    use super::Expression::{Apply, Ref};
    ...
    fn expression_format_apply() {
...

        {
            ...
            let expression = Apply(mk_name("f"), vec![Ref(mk_name("a")), Ref(mk_name("b"))]);
    ...
    fn statement_format_seq() {
...

        {
            ...
            assert_eq!("x2 := x1; x3 := x2", format!("{}", statement));
        }

        {
            ...
            assert_eq!("x2 := x1; x3 := x2; x4 := x3", format!("{}", statement));
        }
    }
    ...
    fn statement_format_while() {
        ...
        assert_eq!("while(p(x)) { x := f(x) }", format!("{}", statement));
    }
}
